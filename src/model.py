...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch, _):
        x, y = batch
        y = F.one_hot(y, num_classes=self.params.n_classes).float()
        ...
        # Apply mixup
        if (
            self.params.training == "mixup"
            and np.random.rand(1) <= self.params.mix_prob
        ):
            x, y = self._mixup(x, y)

        # Apply cutmix
        if (
            self.params.training == "cutmix"
            and np.random.rand(1) <= self.params.mix_prob
        ):
    ...
    def configure_optimizers(self):
        optimizer = get_optimizer(self.parameters(), self.params)
        self.optimizer = optimizer  # Make the optimizer availble to access current lr
        scheduler = get_scheduler(optimizer, self.params)
    ...
    def _mixup(self, x, y):
        lam = np.random.beta(self.params.mix_alpha, self.params.mix_alpha)
